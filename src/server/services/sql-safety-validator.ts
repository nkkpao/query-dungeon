...
export class SqlSafetyValidator {
  validate(sql: string, options: {maxBytes: number}): SqlSafetyResult {
    return validateSqlSafety(sql, options.maxBytes);
  }
}
...
export function validateSqlSafety(sql: string, maxBytes: number): SqlSafetyResult {
  ...
  return {ok: true, sqlHash: hash, errorCode: null, errorMessage: null};
}

function fail(hash: string, errorMessage: string): SqlSafetyResult {
  return {ok: false, sqlHash: hash, errorCode: 'safety_rejected', errorMessage};
}
