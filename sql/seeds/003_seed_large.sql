...
SELECT gs,
       ...
       TIMESTAMPTZ '2022-02-01' + (gs % 720) * INTERVAL '1 day'
FROM generate_series(50001, 200000) AS gs;
...
SELECT gs,
       ...
       CASE
         ...
         ELSE 'paid'
       END,
       TIMESTAMPTZ '2022-06-01' + (gs % 900) * INTERVAL '1 day' + (gs % 24) * INTERVAL '1 hour',
       1000 + ((gs * 97) % 100000)
FROM generate_series(100001, 500000) AS gs;
...
WHERE o.id BETWEEN 100001 AND 500000;
...
FROM orders
WHERE id BETWEEN 100001 AND 500000;
...
SELECT 1 + (gs % 50000), 1 + (gs % 20000), 1 + (gs % 5), 'Review ' || gs,
       TIMESTAMPTZ '2022-07-01' + (gs % 720) * INTERVAL '1 day'
FROM generate_series(100001, 500000) AS gs;
...
SELECT 1 + (gs % 20000),
       ...
       1 + (gs % 12),
       TIMESTAMPTZ '2022-05-01' + (gs % 900) * INTERVAL '1 day'
FROM generate_series(120001, 600000) AS gs;

INSERT INTO user_events (user_id, event_type, metadata, created_at)
SELECT CASE WHEN gs % 4 = 0 THEN 42 ELSE 1 + (gs % 50000) END,
...
SELECT 1 + (gs % 50000),
       ...
       TIMESTAMPTZ '2024-09-01' + (gs % 180) * INTERVAL '1 day',
       CASE WHEN gs % 4 IN (2,3) THEN TIMESTAMPTZ '2024-09-01' + (gs % 180) * INTERVAL '1 day' + INTERVAL '2 days' ELSE NULL END
FROM generate_series(30001, 120000) AS gs;
