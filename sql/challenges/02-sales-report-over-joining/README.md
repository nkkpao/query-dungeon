...
- The baseline joins tables that do not change the requested metric.
- Watch row counts before aggregation.
- Remove nonessential joins before choosing indexes.

Trade-offs: the reference status index helps paid-order reporting but has low value for broad status filters.
