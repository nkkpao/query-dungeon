...
- US1 test tasks T010 through T016 can be drafted in parallel.
- US2 test tasks T027 through T029 can be drafted in parallel after Foundation.
- US3 test tasks T033 through T036 can be drafted in parallel after Foundation.
- Documentation tasks T041 and T042 can run in parallel with final regression task T043.
...
Task: "T012 [P] [US1] Add full correct SubmissionEvaluator happy-path test with persisted metrics and leaderboardEligible=true in tests/submission-evaluation.test.ts"
Task: "T013 [P] [US1] Add 10-run repeatability test for same SQL correctness, normalized rows, row count, and metric presence in tests/submission-evaluation.test.ts"
```

## Parallel Example: User Story 2

```bash
Task: "T027 [P] [US2] Add repository leaderboard tests for correct-only filtering and ranking ties in tests/submission-repository.test.ts"
Task: "T028 [P] [US2] Add route-level leaderboard tests excluding failed and incorrect submissions in tests/server-routes.test.ts"
Task: "T029 [P] [US2] Add API contract expectations for leaderboard metrics and ordering in tests/server-contract.test.ts"
```

## Parallel Example: User Story 3

```bash
Task: "T033 [P] [US3] Add direct SubmissionEvaluator tests for stored submissionId input in tests/submission-evaluation.test.ts"
Task: "T034 [P] [US3] Add direct SubmissionEvaluator tests for explicit evaluation input in tests/submission-evaluation.test.ts"
Task: "T036 [P] [US3] Add route handler test proving routes delegate services without evaluator logic in tests/server-routes.test.ts"
```
